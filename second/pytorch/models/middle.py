...
@register_middle
class SparseMiddleExtractor_dilate(nn.Module):
    def __init__(self,
                 output_shape,
                 use_norm=True,
                 num_input_features=128,
                 num_filters_down1=[64],
                 num_filters_down2=[64, 64],
                 name='SparseMiddleExtractor_dilate'):
        super(SparseMiddleExtractor_dilate, self).__init__()
        self.name = name
        if use_norm:
            BatchNorm1d = change_default_args(
                eps=1e-3, momentum=0.01)(nn.BatchNorm1d)
        ...
        middle_layers.append(nn.ReLU())
        self.middle_conv = spconv.SparseSequential(*middle_layers)
        self.out1 = spconv.SparseConv3d(num_filters[-1],num_filters[-1]/2,(3, 1, 1), (1, 1, 1),dilation=1,bias=False)
        self.out2 = spconv.SparseConv3d(num_filters[-1],num_filters[-1]/2,(3, 1, 1), (1, 1, 1),dilation=2,bias=False)
        self.out3 = spconv.SparseConv3d(num_filters[-1],num_filters[-1]/2,(3, 1, 1), (1, 1, 1),dilation=3,bias=False)
        self.batchN = BatchNorm1d(num_filters[-1]/2*3)
        self.ac = nn.ReLU()
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        return ret
@register_middle
class SpMiddleFHD_dilate(nn.Module):
    def __init__(self,
                 output_shape,
                 use_norm=True,
                 num_input_features=128,
                 ...
                 num_filters_down2=[64, 64],
                 name='SpMiddleFHD'):
        super(SpMiddleFHD_dilate, self).__init__()
        self.name = name
        if use_norm:
            BatchNorm2d = change_default_args(
                eps=1e-3, momentum=0.01)(nn.BatchNorm2d)
        ...
        self.out1 = SubMConv3d(64, 22, 3, dilation=1)
        self.out2 = SubMConv3d(64, 21, 3, dilation=2)
        self.out3 = SubMConv3d(64, 21, 3, dilation=3)
        #self.batchN = BatchNorm1d(64)
        self.ac = nn.ReLU()
        # self.f_conv = spconv.SparseSequential(SpConv3d(63, 64, (3, 1, 1),
        #              (1, 1, 1)),  # [200, 150, 5] -> [200, 150, 2]
        #     BatchNorm1d(64),
        #     nn.ReLU(),
        # )
        # self.grid = torch.full([self.max_batch_size, *sparse_shape], -1, dtype=torch.int32).cuda()
...
@register_middle
class SpMiddleFHD_full3d(nn.Module):
    def __init__(self,
                 output_shape,
                 use_norm=True,
                 num_input_features=128,
        ...
        self.full_conv = Sequential(
            torch.nn.Conv3d(64,64,3,[2,1,1],padding=[0, 1, 1],bias=False),
