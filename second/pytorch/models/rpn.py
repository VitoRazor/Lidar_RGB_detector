...
from second.pytorch.models.network_block import ASFF_ssd
...
class RPNNoHeadBase(nn.Module):
    def __init__(self,
                 use_norm=True,
                 num_class=2,
                 layer_nums=(3, 5, 5),  #layer_nums [5, 5]
                 layer_strides=(2, 2, 2), #layer_strides = [1, 2]
                 num_filters=(128, 128, 256), # [64, 128]
                 upsample_strides=(1, 2, 4), # [1, 2]
                 num_upsample_filters=(256, 256, 256), # [128, 128]
                 num_input_features=128,
                 num_anchor_per_loc=2,
                 encode_background_as_zeros=True,
                 use_direction_classifier=True,
...
class RPNNoHeadBase_asff(nn.Module):
    def __init__(self,
                 use_norm=True,
                 num_class=2,
                 layer_nums=(3, 5, 5),  #layer_nums [5, 5]
        ...
        self.deblocks = nn.ModuleList(deblocks)
        if self.asff:
            self.level_0_fusion = ASFF_ssd(level=0)
            self.level_1_fusion = ASFF_ssd(level=1)
    ...
    def forward(self, x):
        ...
        #     print("stage_outputs")
        #     print(i,":  ",stage_outputs[i].shape)
        if len(stage_outputs)== 2 and self.asff:
            # print("asff",len(stage_outputs))
            temp =[]
            temp.append(self.level_0_fusion(stage_outputs[1],stage_outputs[0]))
            temp.append(self.level_1_fusion(stage_outputs[1],stage_outputs[0]))
            ups = temp
        if len(ups) > 0:
            x = torch.cat(ups, dim=1)
