...
config_path = Path("../config/eval/cityscapes_deeplab_v3_plus.yaml")
tta_flag = args.tta
vis_flag = args.vis
tta_flag = True
...
if vis_flag:
    ...
    with torch.no_grad():
        for batched in tqdm(valid_loader):
            images, _, preds_np, names = predict(batched)
            ...
            for name,img,pred in zip(names,images,preds_np):
                axes[0].imshow(minmax_normalize(img, norm_range=(0, 1), orig_range=(-1, 1)))
                axes[1].imshow(pred)
