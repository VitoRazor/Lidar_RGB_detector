...
class CityscapesDataset(Dataset):
    ...
    def __init__(self, base_dir='../data/cityscapes', split='train',kitti = None,
        ...
        if kitti is not None:
            self.base_dir = Path(kitti)
            self.img_paths = sorted(self.base_dir.glob(f'training/image_2/*.png'))
        else:
